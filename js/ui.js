...
export const closeHelpModalBtn = document.getElementById("close-help-modal-btn");
...
export const droneStatusContent = document.getElementById("drone-status-content");
...
export const botCapacityDisplay = document.getElementById("bot-capacity-display");
...
export function initSettingsListeners() {
    ...
    document.addEventListener("click", (e) => {
        ...
        if (inventoryDrawer.classList.contains("open")) {
            ...
            if (!inventoryDrawer.contains(e.target) && !inventoryFab.contains(e.target)) {
                toggleInventory();
            }
        }
    });
...
export function renderInventoryDrawer() {
    ...
    const createRow = (k, v, isSeed) => {
        ...
        row.className = "bg-[#27272a] p-2 rounded flex items-center border border-[#3f3f46] mb-2";
        row.innerHTML = `
        ...
        trashBtn.className = "text-xs p-1 hover:bg-red-500/20 rounded transition-colors grayscale hover:grayscale-0 opacity-50 hover:opacity-100";
    ...
    // 1. Resources
    const resources = Object.entries(gameState.farmResources).filter(([k, v]) => v > 0 && !k.includes("SEED") && k !== "WATER");
    ...
    // 2. Seeds
    const seeds = Object.entries(gameState.farmResources).filter(([k, v]) => v > 0 && k.includes("SEED"));
    if (seeds.length > 0) {
        const h3 = document.createElement("h3");
...
export function handleDiscard(itemType, amount = null) {
    ...
    if (gameState.farmResources[itemType] >= qty) {
        ...
        if (cropInfo[itemType] && !itemType.endsWith("_SEED")) {
             gameState.drone.inventory[itemType] = Math.max(0, (gameState.drone.inventory[itemType] || 0) - qty);
             if (gameState.drone.inventory[itemType] === 0) delete gameState.drone.inventory[itemType];
        }
...
export function renderDroneStatus() {
    const invCount = Object.values(gameState.drone.inventory).reduce((a,b)=>a+b,0);
    ...
    droneStatusContent.innerHTML = `
        ...
        <div class="mt-4">
            ...
            <div class="flex flex-wrap gap-1">
                ${Object.entries(gameState.drone.inventory).map(([k,v]) => `
...
export function render() {
    ...
    runBtn.textContent = gameState.isAutoRunning ? "PARAR (Auto)" : (gameState.isRunning ? "Executando..." : "EXECUTAR");
    runBtn.className = gameState.isAutoRunning ? "btn btn-danger w-full" : "btn btn-primary w-full";
...
export function logToConsole(msg, type="log") {
    ...
    p.textContent = `> ${msg}`;
    p.style.color = type === "error" ? "#ef4444" : type === "warn" ? "#f59e0b" : type === "info" ? "#3b82f6" : "#a1a1aa";
...
export function updateDroneVisualPosition(x, y, width, height, animated = true) {
    ...
    droneEl.style.transition = animated ? `transform ${speed/1000}s linear` : 'none';
...
window.addEventListener('resize', () => {
    if (gameState.drone) {
        updateDroneVisualPosition(gameState.drone.x, gameState.drone.y, gameState.gridSize.width, gameState.gridSize.height, false);
    }
});
...
export function updateTechModalContent(category, buyCallback) {
    ...
    const items = Object.entries(gameState.upgrades).filter(([,u]) => u.category === category);
    ...
    items.forEach(([id, u]) => {
         const canAfford = Object.entries(u.cost).every(([k,v]) => gameState.farmResources[k] >= v);
         const costStr = Object.entries(u.cost).map(([k,v]) => {
             const icon = cropInfo[k.replace("_SEED","")]?.emoji || k;
             const clr = gameState.farmResources[k] >= v ? 'text-green-400' : 'text-red-400';
             return `<span class="${clr}">${v} ${icon}</span>`;
         }).join(" ");
         
         const btnClass = u.unlocked ? "btn-secondary opacity-50" : canAfford ? "btn-primary" : "btn-secondary opacity-50";
         const btnText = u.unlocked ? "Comprado" : "Comprar";
         
         const div = document.createElement("div");
         div.className = "p-4 bg-[#27272a] rounded flex justify-between items-center border border-[#3f3f46]";
         div.innerHTML = `
            <div>
                <h4 class="font-bold text-white">${u.name}</h4>
                <p class="text-sm text-gray-400">${u.description}</p>
                <div class="mt-2 text-xs font-mono bg-[#18181b] p-1 rounded inline-block">
                    ${costStr}
                </div>
            </div>
            <button class="btn ${btnClass}" ${!canAfford || u.unlocked ? 'disabled' : ''}>
                ${btnText}
            </button>
         `;
...
export function renderFunctionList() {
    ...
    Object.keys(apiFunctionsData).forEach(funcName => {
        const btn = document.createElement("button");
        btn.className = "w-full text-left p-2 hover:bg-[#18181b] rounded text-sm text-gray-400 hover:text-white transition-colors border-b border-white/5";
...
export function displayFunctionDetails(funcName) {
    ...
    functionDetailsEl.innerHTML = `
        <h3 class="text-xl font-bold text-accent-primary mb-1">${funcName}</h3>
        <code class="block bg-[#18181b] p-2 rounded text-xs text-yellow-500 mb-4">${data.signature}</code>
        <p class="text-white mb-4">${data.description}</p>
        <div class="bg-[#18181b] p-3 rounded border-l-4 border-accent-secondary">
            <h4 class="text-xs font-bold text-gray-300 uppercase mb-1">Detalhes Adicionais</h4>
            <p class="text-xs text-white">${data.details}</p>
        </div>
    `;
    
    // Highlight active in list
    Array.from(functionListEl.children).forEach(btn => {
        if (btn.textContent === funcName) btn.classList.add("text-white", "bg-[#18181b]");
        else btn.classList.remove("text-white", "bg-[#18181b]");
    });
}
...
export function renderBotView(buyCallback, toggleCallback) {
    ...
    // 1. Update Header
    botStatusBadge.textContent = gameState.botFarm.isRunning ? "OPERACIONAL" : "PAUSADO";
    ...
    if (currentBotTab === 'active') {
        ...
        if (activeBots.length === 0) {
            ...
            activeBots.forEach((bot) => { // Use original index? No, use ID for toggle.
                const info = cropInfo[bot.type];
                const pct = Math.min(100, ((bot.progress || 0) / info.growthTime) * 100);
        ...
        gameState.unlockedCrops.forEach(cropType => {
            ...
            const ownedStats = gameState.botFarm.robots.filter(b => b.type === cropType).length;
...
export function openDeleteModal(item) {
    ...
    const emoji = item.includes("_SEED") ? (info.seedEmoji || '🌱') : (info.emoji || '📦');
...
export function initViewportInteractions() {
    ...
    // Zoom (Wheel)
    gameViewportEl.addEventListener("wheel", (e) => {
        e.preventDefault();
...
function updateViewportTransform() {
    ...
    const gridW = parseInt(farmGridEl.style.width) || (gameState.gridSize.width * TILE_SIZE);
    const gridH = parseInt(farmGridEl.style.height) || (gameState.gridSize.height * TILE_SIZE);
