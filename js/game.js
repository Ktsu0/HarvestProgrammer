...
export function handleBuyUpgrade(upgradeID, activeTab) {
  ...
  if (canAfford) {
    ...
    if (activeTab) {
      UI.updateTechModalContent(activeTab, (id) =>
        handleBuyUpgrade(id, activeTab),
      );
    }
    UI.render();
...
export function validateCode(code) {
  ...
  // 5. Check for Variables (let, const, var) - Advanced logic blockade
  if (!gameState.codeFeatures.variables) {
    const varRegex = /\b(let|const|var)\s+[a-zA-Z0-9_]+\s*=/g;
    if (varRegex.test(code)) {
      throw new Error(
        "[CODE_BLOCKED] O uso de Variáveis (`let`, `const`, `var`) está bloqueado. Compre o upgrade 'CODE_VARIABLES'.",
      );
    }
  }

  // 6. Check for Data Structures (Simple Array check)
  if (!gameState.codeFeatures.structures) {
    if (code.includes("[") || code.includes("]")) {
      throw new Error(
        "[CODE_BLOCKED] O uso de Arrays ([]) está bloqueado. Compre o upgrade 'CODE_STRUCTURES'.",
      );
    }
  }

  // 7. Check for Math Library
  if (!gameState.codeFeatures.math) {
    if (code.includes("Math.")) {
      throw new Error(
        "[CODE_BLOCKED] O uso da biblioteca `Math` está bloqueado. Compre o upgrade 'CODE_MATH'.",
      );
    }
  }

  // 8. Security/Immersion check
  ...
  for (const glob of forbiddenGlobals) {
    if (code.includes(glob)) {
      throw new Error(
        `[SECURITY_BLOCKED] O acesso ao objeto '${glob}' está restrito pelo Firewall do Drone.`,
      );
    }
  }
}
...
export function gameLoop() {
  ...
  if (now - gameState.lastRainCheck > 10000) {
    ...
    if (Math.random() * 100 < gameState.rainChance) {
      UI.logToConsole(
        "🌧️ Está chovendo! Plantas regadas e aceleradas.",
        "info",
      );
      ...
      gameState.farmResources["WATER"] = Math.min(
        ...
        gameState.farmResources["WATER"] + bonusWater,
      );
      ...
      setTimeout(() => {
        gameState.grid.forEach((row) =>
          row.forEach((tile) => (tile.isRaining = false)),
        );
        UI.render();
      }, 5000);
    }
    ...
    if (delta > 1000) {
      ...
      gameState.botFarm.robots.forEach((bot) => {
        ...
        if (bot.progress >= info.growthTime) {
          ...
          gameState.farmResources[bot.type] =
            (gameState.farmResources[bot.type] || 0) + amount;
          ...
          if (nextSeedType) {
            ...
            if (Math.random() * 100 < dropChance) {
              gameState.farmResources[nextSeedType] =
                (gameState.farmResources[nextSeedType] || 0) + 1;
            }
          }
    ...
    row.forEach((tile) => {
      if (tile.state === "PLANTED" && tile.crop) {
        ...
        if (info) {
          ...
          const requiredTime = tile.watered
            ? info.growthTime
            : info.growthTime * 3;
...
export async function runSingleCycle(API_INSTANCE) {
  ...
  try {
    ...
    const savedLogs = children.filter(
      (p) =>
        p.className.includes("text-red") ||
        p.className.includes("text-yellow") ||
        p.className.includes("text-blue"),
    );
    ...
    if (gameState.isAutoRunning) {
      ...
      UI.logToConsole(
        "Execução automática interrompida devido a erro.",
        "error",
      );
    }
  }
...
export function buyRobot(cropType) {
  ...
  const ownershipCount = gameState.botFarm.robots.filter(
    (b) => b.type === cropType,
  ).length;
  ...
  if ((gameState.farmResources[cropType] || 0) < cost) {
    UI.logToConsole(
      `Precisa de ${cost} ${cropType} para este Robô (Você tem ${ownershipCount} deste tipo).`,
      "error",
    );
    return false;
  }
  ...
  UI.logToConsole(
    `Robô de ${cropType} comprado por ${cost}! ${canActivate ? "Já está operando." : "Enviado para o Armazém."}`,
    "info",
  );
  UI.render();
  return true;
...
export function sellRobot(index) {
  ...
  const refund = 25;
  gameState.farmResources[bot.type] =
    (gameState.farmResources[bot.type] || 0) + refund;
